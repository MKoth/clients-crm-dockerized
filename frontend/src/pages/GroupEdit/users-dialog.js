...
export const UsersDialog = (props) => {
  ...
  return (
    <Dialog onClose={(e)=>{console.log(e)}} aria-labelledby="users-dialog-title" open={open}>
      ...
      <DialogContent style={{padding:0}}>
        <List>
          ...
          {users.map((user) => (
            <ListItem button onClick={(user) => onClose([user])} key={user.id}>
              <ListItemIcon>
                <Checkbox
                  onClick={(e)=>{e.stopPropagation()}}
              ...
              <ListItemAvatar>
                <Avatar>
                  {user.image?<img src={user.image} style={{width:'100%'}}/>:<PersonIcon />}
                </Avatar>
              </ListItemAvatar>
